...
- Classify the content that Kaede's protocols handle (instead of using redundant arguments, make them easier to handle by using classes or data classes, such as DNSRecord for DNS records and HTTPMessage for HTTP messages). This does not mean classifying aggressively wherever possible — apply it to the data/content being processed, not to every piece of logic.
