...
from kaede.udp.api.server import UDPGate, UDPServerLimits

# UDPGate is admission control for peers rather than for connections: a global
...
class TestConnectionRate:
    ...
    def test_peers_are_counted_separately(self):
        # A datagram's source address is trivially forged, so this only limits
        # the honest case. It must still not let one address spend another's budget.
        g = gate(rate=[(1, 1)])
