...
from kaede.tcp.api.server import TCPGate, TCPServerLimits

# TCPGate is admission control: a global cap on concurrent connections and a
# per-peer sliding-window cap on new connections. The clock is injected so the
# windows can be checked exactly rather than by sleeping.
...
class TestConnectionRate:
    ...
    def test_every_window_is_enforced(self):
        # With [(1, 5), (60, 6)] a peer may burst 5 per second but only 6 per minute.
    ...
    def test_a_refused_connection_is_not_recorded(self):
        # A rejected attempt must not consume budget, or a blocked peer could
        # never recover while it keeps retrying.
        g = gate(rate=[(10, 1)])
